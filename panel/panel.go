...
func (p *Panel) loadCore(panelConfig *Config) *core.Instance {
	...
	config := &core.Config{
		App: []*serial.TypedMessage{
			...
			// mydispatcher registers as routing.DispatcherType() and so
			// replaces the upstream app/dispatcher. We must NOT also
			// register dispatcher.Config{} here, or both would claim the
			// same feature key.
...
func (p *Panel) Start() {
	...
	for _, nodeConfig := range p.panelConfig.NodesConfig {
		...
		// Hysteria used to live on the sing-box controller, but xray-core
